...
### Requirement: User SHALL view variant cost history

The variant cost history view SHALL display all cost records for the variant including amount, effective date, and superseded status.

#### Scenario: View cost history
- **WHEN** user clicks "Ver historial de costos" button on variant card
...
- **WHEN** variant has only current cost
