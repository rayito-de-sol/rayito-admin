...
import type { CreateOrderRequest, OrderSource } from '@/types/order'
import { toast } from '@/utils/toast'
import { Trash2, Plus } from 'lucide-react'

// Currently only manual sources are exposed here; extend as needed
const MANUAL_SOURCES: { value: OrderSource; label: string }[] = [
...
/**
 * ManualOrderModal
 * Form to record a manual order (WhatsApp, etc.)
 * Source dropdown is extensible for future channels.
 */
export const ManualOrderModal = ({ open, onClose, onSuccess }: ManualOrderModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      await ordersService.createOrder(req)
